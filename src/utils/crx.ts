...
function getPublicKeyFromProtoBuf(
  ...
  while (startOffset < endOffset) {
    ...
    if (keyproofkey !== 0xa) {
      startOffset += keyprooflength;
      console.warn(
        'proto: Clave inesperada en AsymmetricKeyProof: ' + keyproofkey,
      );
      continue;
    }
    if (startOffset + keyprooflength > endOffset) {
      console.warn('proto: el tamaño de  public_key es muy largo');
      break;
    }
  ...
  if (!publicKeys.length) {
    console.warn('proto: No se pudo encontrar ninguna clave publica');
    return null;
  }
  if (!crxIdBin) {
    console.warn('proto: No se pudo encontrar crx_id');
    return null;
  }
  ...
  console.warn('proto: Ninguna de las claves publicas combina con crx_id');
...
export const parseCrx = (buf: Buffer) => {
  ...
  if (buf[0] !== 67 || buf[1] !== 114 || buf[2] !== 50 || buf[3] !== 52) {
    throw new Error('Encabezado inválido: no inicia con Cr24');
  }

  // 02 00 00 00
  if ((buf[4] !== 2 && buf[4] !== 3) || buf[5] || buf[6] || buf[7]) {
    throw new Error('Formato de version crx inesperado.');
  }
