...
export class SessionsService {
  ...
  public constructor() {
    ...
    */
    ...
    const store = require('data-store')({ path: process.cwd() + '/downloads.json' });
    ...
    function downloadsData(){
      if(store.get("downloads") == undefined){
    ...
    const downloads: IDownloadItem[] = Array.from(store.get("downloads"));
    ...
    // TODO(sentialx): clean up he download listeners
    this.view.on('will-download', (event, item, webContents) => {
      ...
      item.on('updated', (event, state) => {
        if (state === 'interrupted') {
          console.log('Descarga interrumpida pero puede ser resumida');
        ...
        if (state === 'completed') {
          ...
          if (process.env.ENABLE_EXTENSIONS && extname(fileName) === '.crx') {
            ...
            if (await pathExists(path)) {
              console.log('La extensión ya está instalada');
              return;
            }
