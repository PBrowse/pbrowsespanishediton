...
const Fields = observer(() => {
  return (
    <div style={{ display: store.content !== 'list' ? 'block' : 'none' }}>
      <Textfield ref={store.usernameRef} label="Usuario" />
...
export const App = observer(() => {
  ...
  if (store.content === 'list') {
    title = store.list.length
      ? 'Guardar contraseñas en este sitio'
      : 'No hay contraseñas guardadas de este sitio';
  } else {
    title = store.content === 'save' ? '¿Guardar contraseña?' : '¿Actualizar contraseña?';
  }
