...
const onContextMenu = (tab: ITab) => () => {
  const menu = remote.Menu.buildFromTemplate([
    ...
    {
      label: tab.isPinned ? 'Quitar fijado de pestaña' : 'Fijar pestaña',
      click: () => {
        tab.isPinned ? store.tabs.unpinTab(tab) : store.tabs.pinTab(tab);
      },
    },
    {
      label: tab.isMuted ? 'Quitar silenciado de la pestaña' : 'Silenciar pestaña',
      click: () => {
        tab.isMuted ? store.tabs.unmuteTab(tab) : store.tabs.muteTab(tab);
      },
    },
    ...
    {
      label: 'Revertir pestañas cerradas',
